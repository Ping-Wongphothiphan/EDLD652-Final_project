...
```{r}
plot1<-data %>% ggplot(aes(GenConBlf, VacIntMMR))+
  geom_point(aes(color = Condition), alpha = .3, position = "jitter")+
...
data %>% 
  ...
  geom_point(data = filter(data, Condition == "Control"), color = "orchid", position = "jitter")+
