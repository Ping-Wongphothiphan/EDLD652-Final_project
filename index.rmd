...
### Animated plot for **risk perception of MMR vaccinae** of each message format
...
### Grouped by **age and preferred political parties** {data-commentary-width=600}
...
plot6.anim <- data %>% group_by(SlfPolIdeo, age) %>% 
  ...
  pivot_longer(!c(SlfPolIdeo, age), 
               ...
               values_to = "score") %>%
  mutate(ConBlfTypes = recode(ConBlfTypes, "avg_GenConBlf" = "GENERAL", "avg_VacConBlf" = "VACCINATION")) %>% 
...
### By gender {data-commentary-width=600}
...
### By income {data-commentary-width=600}
...
1) I factor the order of levels of "income" variable so that it is organized from low to high.

2) I added geom_jitter() in addition to geom_boxplot to visualize the density of data in each level and categorized them by colors.

3) I merged the two plots by animiation function to visualize the differences of conspiracy beliefs in general vs conspiracy beliefs in vaccination across groups of income.
...
### By race {data-commentary-width=400}
...
plot5_anim <- data %>% 
  ...
  labs(title = "Which race has higher level of conspiracy beliefs in {closest_state}?", 
       x = "Level of general conspiracy belief",
       y = "Density")+ 
  scale_x_continuous(labels = c("None", 
                                " ", 
...
### By Education {data-commentary-width=400}
...
plot3.anim <-
  ...
  labs(title = "Does education matters for individual conspiracy beliefs?",
       subtitle = "Showing conspiracy beliefs in {closest_state}",
       x = "Level of conspiracy beliefs")+
  scale_x_continuous(labels = c("None", 
                                " ", 
